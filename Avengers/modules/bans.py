...
from Avengers import (DEV_USERS, LOGGER, OWNER_ID, DRAGONS, DEMONS, TIGERS,
                          WOLVES, dispatcher)
from Avengers.modules.disable import DisableAbleCommandHandler
from Avengers.modules.helper_funcs.chat_status import (
    bot_admin, can_restrict, connection_status, is_user_admin,
    is_user_ban_protected, is_user_in_chat, user_admin, user_can_ban)
from Avengers.modules.helper_funcs.extraction import extract_user_and_text
from Avengers.modules.helper_funcs.string_handling import extract_time
from Avengers.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text(
                "Trying to put me against my God!")
            return log_message
            ...
            message.reply_text(
                "Bring an order from Bot Lab to fight a Demon disaster."
            )
            return log_message
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order Bot Lab to fight a Tiger disaster."
            )
            return log_message
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Deafeated! {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML)
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#KICKED\n"
