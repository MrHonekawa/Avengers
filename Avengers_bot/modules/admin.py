...
from Avengers_bot import DRAGONS, dispatcher
from Avengers_bot.modules.disable import DisableAbleCommandHandler
from Avengers_bot.modules.helper_funcs.chat_status import (bot_admin, can_pin,
                                                           can_promote,
                                                           connection_status,
                                                           user_admin,
                                                           ADMIN_CACHE)

from Avengers_bot.modules.helper_funcs.extraction import (extract_user,
                                                          extract_user_and_text)
from Avengers_bot.modules.log_channel import loggable
from Avengers_bot.modules.helper_funcs.alternate import send_message
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    bot.sendMessage(
        chat.id,
        f"Sucessfully promoted <b>{user_member.user.first_name or user_id}</b>. Now Give me a Party Now🥳🥳!",
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status == 'creator':
        message.reply_text(
            "This is the person who CREATED this chat, how would I demote them?")
        return

    if not user_member.status == 'administrator':
        message.reply_text("I Can't demote what wasn't promoted!")
        return
    ...
    try:
        ...
        bot.sendMessage(
            chat.id,
            f"I Sucessfully demoted <b>{user_member.user.first_name or user_id}</b>!",
            parse_mode=ParseMode.HTML)
